...
std::string CommController::getEmailCode(std::string& email) {
	...
	if (vv.has_value() && vv.value() == 0) {
		...
		return bzh::conversionJsonOfMsg<bzh::http_status::success>("email seed success");
	} else {
		return bzh::conversionJsonOfMsg<bzh::http_status::error>("email is occupied");
	}
}

std::string CommController::getLanguageList() {
	auto language_list = userMapper.getLanguageList();
	return bzh::conversionJsonOfCus<bzh::http_status::success>([&language_list](nlohmann::json& json) {
		json = language_list;
		}, "language type list");
}
