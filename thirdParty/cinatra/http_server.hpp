...
namespace cinatra {
	...
	public:
		...
		//don't begin with "./" or "/", not absolutely path  absolute_path:绝对路径
		void set_public_root_directory(const std::string& name, const std::string& _absolute_path = "") {
			if (!name.empty()) {
				public_root_path_ = "./" + name + "/";
			} else {
				public_root_path_ = "./";
			}
			absolute_path_ = _absolute_path;
		}
	...
	private:
			...
			set_http_handler<POST, GET>(STATIC_RESOURCE, [this](request& req, response& res) {
				...
				switch (state) {
					case cinatra::data_proc_state::data_begin:
					{
						...
						//添加了绝对路径
						if (absolute_path_ != "") {
							auto prp_size = public_root_path_.size();
							relatice_file_name.replace(0, prp_size, absolute_path_);
						}
		...
		std::string absolute_path_ = ""; //绝对路径
